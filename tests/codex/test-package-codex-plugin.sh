...
assert_contains "$archive_paths" "skills/controller-gated-mpd/SKILL.md" "archive includes Controller-Gated MPD skill"
assert_contains "$archive_paths" "skills/controller-gated-mpd/agents/openai.yaml" "archive includes Controller-Gated MPD OpenAI metadata"
source_cgmpd_metadata="$(cat "$REPO_ROOT/skills/controller-gated-mpd/agents/openai.yaml")"
archive_cgmpd_metadata="$(read_archive_file "$archive" skills/controller-gated-mpd/agents/openai.yaml)"
assert_equals "$archive_cgmpd_metadata" "$source_cgmpd_metadata" "source-owned Controller-Gated MPD metadata is preserved"
...
assert_equals "$manifest_summary" "adaptive-superpowers	$expected_version	./skills/	$source_hooks" "archive manifest preserves adaptive identity and source hooks"
